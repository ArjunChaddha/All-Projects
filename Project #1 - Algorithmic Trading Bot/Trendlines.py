...
from SupportFile4 import getPointsBest
import math
from scipy.optimize import minimize
from SupportFile6 import run_
...
class Line:
    def __init__(self, x1, y1, x2, y2, type=None) -> None:
        self.xMin = x1
        self.xMax = x2
        self.y1 = y1
        self.y2 = y2
...
class Scorer:
    ...
    def plot(self, showLows=True):
        if showLows:
            plt.plot(self.xScaled, self.yScaled, '-o', markevery=self.lowsUnscaled, color='red')
            # plt.show()
        else:
            plt.plot(self.xScaled, self.yScaled, color='red')
            # plt.show()
    ...
    def plotFinalWithDotted(self, lines, showLows=True, tillEnd=False, showHighs=False, highs=None):
        ...
        mng.full_screen_toggle()
        ax1.set_ylim([0.8*min(data["low"]), 1.1*max(data["high"])])
        #HERE CHANGE THIS TO ADD A TITLE TO THE STOCK
        plt.title('AMZN' + " Trendlines from 2020-2022")
...
if __name__ == "__main__":
    #Enter stock ticker, start date, and end date
    STOCK = 'MSFG'
    startdate='2020-01-01'
    endDate = '2022-10-27'

    data, highs, lows = getPointsBest(STOCK, max_=0.4, limit=70, min_=0.15, startDate=startdate, endDate=endDate, interval=intervalSET, returnData='lows')
    # data = -1*data + 500
    print(data.head())
    ...
    data, highs, lows = getPointsBest(STOCK, max_=0.4, limit=70, min_=0.15, startDate=startdate, endDate=endDate, interval=intervalSET, returnData='highs')
    # data = -1*data + 500
    print(data.head())
