...
class FinPolyLine2(pg.PolyLineROI):
    def __init__(self, vb, *args, **kwargs):
    ...
    def segmentClicked(self, segment, ev=None, pos=None):
        print("I HAVE BEEN CLICKED YESSSSS")
...
def create_plot(title='Finance Plot', rows=1, init_zoom_periods=1e10, maximize=True, yscale='linear', top_graph_scale=2):
...
def add_point(p0, color=draw_line_color, width=1, style=None, interactive=False, ax=None):
    p1 = (p0[0]+1, p0[1])
    ax = _create_plot(ax=ax, maximize=False)
    used_color = _get_color(ax, style, color)
    ...
    if interactive:
        line = FinPolyLine2(ax.vb, pts, closed=True, pen=pen, movable=False)
        ax.vb.rois.append(line)
    else:
        line = FinLine(pts, pen=pen)
    # print("Added line here", line.pos())
    line.ax = ax
    ax.addItem(line)
    return line
